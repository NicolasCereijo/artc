...
def test_compare_two_sp_contrast(setup):
    ...
    assert analysis.compare_two_spect_contrast(audio_signal3, audio_signal3,
                                               ...
                                               hop_length=hop_length) == 1
    assert round(analysis.compare_two_spect_contrast(audio_signal1, audio_signal2,
                                                     sample_rate1, sample_rate2,
                                                     hop_length=hop_length), 5) == 0.73822
    assert round(analysis.compare_two_spect_contrast(audio_signal2, audio_signal1,
                                                     sample_rate2, sample_rate1,
                                                     hop_length=hop_length), 5) == 0.73822
    assert round(analysis.compare_two_spect_contrast(audio_signal1, audio_signal3,
                                                     sample_rate1, sample_rate3,
                                                     hop_length=hop_length), 5) == 0.74308
    assert round(analysis.compare_two_spect_contrast(audio_signal3, audio_signal1,
                                                     sample_rate3, sample_rate1,
                                                     hop_length=hop_length), 5) == 0.74308
    assert round(analysis.compare_two_spect_contrast(audio_signal2, audio_signal3,
                                                     sample_rate2, sample_rate3,
                                                     hop_length=hop_length), 5) == 0.75110
    assert round(analysis.compare_two_spect_contrast(audio_signal3, audio_signal2,
...
def test_compare_multiple_sp_contrast(setup):
    ...
    assert analysis.compare_multiple_spect_contrast([audio_signal3, audio_signal3],
                                                    [sample_rate3, sample_rate3],
                                                    hop_length=hop_length) == 1
    assert round(analysis.compare_multiple_spect_contrast([audio_signal1, audio_signal2],
                                                          [sample_rate1, sample_rate2],
                                                          hop_length=hop_length), 5) == 0.73822
    assert round(analysis.compare_multiple_spect_contrast([audio_signal2, audio_signal1],
                                                          [sample_rate2, sample_rate1],
                                                          hop_length=hop_length), 5) == 0.73822
    assert round(analysis.compare_multiple_spect_contrast([audio_signal1, audio_signal3],
                                                          [sample_rate1, sample_rate3],
                                                          hop_length=hop_length), 5) == 0.74308
    assert round(analysis.compare_multiple_spect_contrast([audio_signal3, audio_signal1],
                                                          [sample_rate3, sample_rate1],
                                                          hop_length=hop_length), 5) == 0.74308
    assert round(analysis.compare_multiple_spect_contrast([audio_signal2, audio_signal3],
                                                          [sample_rate2, sample_rate3],
                                                          hop_length=hop_length), 5) == 0.75110
    assert round(analysis.compare_multiple_spect_contrast([audio_signal3, audio_signal2],
                                                          [sample_rate3, sample_rate2],
                                                          hop_length=hop_length), 5) == 0.75110
