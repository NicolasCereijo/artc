...
def test_compare_two_onset_detection(setup):
    data_set = setup
    hop_length = 8192
    ...
    assert analysis.compare_two_onset_detection(audio_signal3, audio_signal3,
                                                ...
                                                hop_length=hop_length) == 1
    assert round(analysis.compare_two_onset_detection(audio_signal1, audio_signal2,
                                                      sample_rate1, sample_rate2,
                                                      hop_length=hop_length), 5) == 0.86024
    assert round(analysis.compare_two_onset_detection(audio_signal2, audio_signal1,
                                                      sample_rate2, sample_rate1,
                                                      hop_length=hop_length), 5) == 0.86024
    assert round(analysis.compare_two_onset_detection(audio_signal1, audio_signal3,
                                                      sample_rate1, sample_rate3,
                                                      hop_length=hop_length), 5) == 0.92788
    assert round(analysis.compare_two_onset_detection(audio_signal3, audio_signal1,
                                                      sample_rate3, sample_rate1,
                                                      hop_length=hop_length), 5) == 0.92788
    assert round(analysis.compare_two_onset_detection(audio_signal2, audio_signal3,
                                                      sample_rate2, sample_rate3,
                                                      hop_length=hop_length), 5) == 0.94430
    assert round(analysis.compare_two_onset_detection(audio_signal3, audio_signal2,
...
def test_compare_multiple_onset_detection(setup):
    ...
    assert analysis.compare_multiple_onset_detection([audio_signal3, audio_signal3],
                                                     [sample_rate3, sample_rate3],
                                                     hop_length=hop_length) == 1
    assert round(analysis.compare_multiple_onset_detection([audio_signal1, audio_signal2],
                                                           [sample_rate1, sample_rate2],
                                                           hop_length=hop_length), 5) == 0.86024
    assert round(analysis.compare_multiple_onset_detection([audio_signal2, audio_signal1],
                                                           [sample_rate2, sample_rate1],
                                                           hop_length=hop_length), 5) == 0.86024
    assert round(analysis.compare_multiple_onset_detection([audio_signal1, audio_signal3],
                                                           [sample_rate1, sample_rate3],
                                                           hop_length=hop_length), 5) == 0.92788
    assert round(analysis.compare_multiple_onset_detection([audio_signal3, audio_signal1],
                                                           [sample_rate3, sample_rate1],
                                                           hop_length=hop_length), 5) == 0.92788
    assert round(analysis.compare_multiple_onset_detection([audio_signal2, audio_signal3],
                                                           [sample_rate2, sample_rate3],
                                                           hop_length=hop_length), 5) == 0.94430
    assert round(analysis.compare_multiple_onset_detection([audio_signal3, audio_signal2],
                                                           [sample_rate3, sample_rate2],
                                                           hop_length=hop_length), 5) == 0.94430
