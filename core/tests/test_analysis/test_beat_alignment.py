...
def test_compare_two_beat_alignment(setup):
    ...
    assert analysis.compare_two_beat_alignment(audio_signal3, audio_signal3,
                                               ...
                                               hop_length=hop_length) == 1
    assert round(analysis.compare_two_beat_alignment(audio_signal1, audio_signal2,
                                                     sample_rate1, sample_rate2,
                                                     hop_length=hop_length), 5) == 0.49534
    assert round(analysis.compare_two_beat_alignment(audio_signal2, audio_signal1,
                                                     sample_rate2, sample_rate1,
                                                     hop_length=hop_length), 5) == 0.49534
    assert round(analysis.compare_two_beat_alignment(audio_signal1, audio_signal3,
                                                     sample_rate1, sample_rate3,
                                                     hop_length=hop_length), 5) == 0.74178
    assert round(analysis.compare_two_beat_alignment(audio_signal3, audio_signal1,
                                                     sample_rate3, sample_rate1,
                                                     hop_length=hop_length), 5) == 0.74178
    assert round(analysis.compare_two_beat_alignment(audio_signal2, audio_signal3,
                                                     sample_rate2, sample_rate3,
                                                     hop_length=hop_length), 5) == 0.33259
    assert round(analysis.compare_two_beat_alignment(audio_signal3, audio_signal2,
...
def test_compare_multiple_beat_alignment(setup):
    ...
    assert analysis.compare_multiple_beat_alignment([audio_signal3, audio_signal3],
                                                    [sample_rate3, sample_rate3],
                                                    hop_length=hop_length) == 1
    assert round(analysis.compare_multiple_beat_alignment([audio_signal1, audio_signal2],
                                                          [sample_rate1, sample_rate2],
                                                          hop_length=hop_length), 5) == 0.49534
    assert round(analysis.compare_multiple_beat_alignment([audio_signal2, audio_signal1],
                                                          [sample_rate2, sample_rate1],
                                                          hop_length=hop_length), 5) == 0.49534
    assert round(analysis.compare_multiple_beat_alignment([audio_signal1, audio_signal3],
                                                          [sample_rate1, sample_rate3],
                                                          hop_length=hop_length), 5) == 0.74178
    assert round(analysis.compare_multiple_beat_alignment([audio_signal3, audio_signal1],
                                                          [sample_rate3, sample_rate1],
                                                          hop_length=hop_length), 5) == 0.74178
    assert round(analysis.compare_multiple_beat_alignment([audio_signal2, audio_signal3],
                                                          [sample_rate2, sample_rate3],
                                                          hop_length=hop_length), 5) == 0.33259
    assert round(analysis.compare_multiple_beat_alignment([audio_signal3, audio_signal2],
                                                          [sample_rate3, sample_rate2],
                                                          hop_length=hop_length), 5) == 0.33259

    assert analysis.compare_multiple_beat_alignment(
