...
def compare_two_spect_flatness(audio_signal1: np.ndarray, audio_signal2: np.ndarray, n_fft: int = 8192) -> float:
    flatness1 = calculate_spectral_flatness(audio_signal1, n_fft)
    flatness2 = calculate_spectral_flatness(audio_signal2, n_fft)
...
def compare_multiple_spect_flatness(audio_signals: list, n_fft: int = 8192) -> float:
    ...
    for i in range(num_signals):
        for j in range(i + 1, num_signals):
            similarity = compare_two_spect_flatness(audio_signals[i], audio_signals[j], n_fft)
