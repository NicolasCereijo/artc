...
def pearson_correlation(array1: np.ndarray, array2: np.ndarray):
...
def cosine_similarity_coefficient(array1: np.ndarray, array2: np.ndarray):
    vector1, vector2 = dt_structs.adjust_dimensions(array1, array2)
...
def normalized_relative_difference_individual(value1: float, value2: float):
    similarity_percentage = 1 - (abs(value1 - value2) / max(value1, value2))
    return similarity_percentage


def normalized_relative_difference_array(array1: np.ndarray, array2: np.ndarray):
