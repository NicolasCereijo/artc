...
def compare_two_chroma(audio_signal1: np.ndarray, audio_signal2: np.ndarray,
    ...
    chroma1 = calculate_chroma(audio_signal1, sample_rate1, n_fft=n_fft)
    chroma2 = calculate_chroma(audio_signal2, sample_rate2, n_fft=n_fft)

    similarity_percentage = comparisons.round_to_one(
        comparisons.pearson_correlation(chroma1, chroma2))
